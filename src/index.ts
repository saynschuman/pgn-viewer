...
export const renderMove =
  ...
  (move: MoveData, isVariation: boolean = false) => {
    return `<span data-ply="${move.ply}" id="move-${move.path.path}" data-nags="${move.nags}" data-fen="${
      ...
      move.path.path
    }" data-variation="${isVariation}" class="${ctrl.path.path === move.path.path ? "active" : ""} move ${hasSymbol(
      move
    )} ${symbolClass(move)} ${isVariation ? "variation" : ""}"> ${move.san} </span>`;
  };
...
export class PgnViewer {
   ...
   */
   public getMovesFromPath(path: string): MoveData[] {
    const result: MoveData[] = [];

    // Start from the given path and move backward to gather moves
    let currentNode: AnyNode | undefined = this.nodeAtPathOrNull(path);
  ...
  */
  public removeAllResultComments(): void {
    ...
    const removeResultCommentsInNode = (node: AnyNode): void => {
      // @ts-ignore
  ...
  removeAllCommentsAndNags(node: AnyNode): void {
    ...
    if (node.children) {
      node.children.forEach((child: AnyNode) => {
