...
export const renderMove = (ctrl) => (move, isVariation = false) => {
    return `<span data-ply="${move.ply}" data-nags="${move.nags}" data-fen="${move.fen}" data-uci="${move.uci}" data-path="${move.path.path}" data-variation="${isVariation}" class="move ${hasSymbol(move)} ${symbolClass(move)} ${isVariation ? "variation" : ""}" id="${ctrl.path.path === move.path.path ? "active" : `move-${move.path.path}`}"> ${move.san} </span>`;
};
